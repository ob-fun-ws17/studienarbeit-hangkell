...
spec = do
  describe "newGame" $ do
    ...
    it "creates simple Game" $
      newGame "a" `shouldBe` Just (Game [('a', False)] [p0] True p0 "")
  describe "makeATurn" $ do
    ...
    it "makes a turn with killing mistake" $
      makeATurn p1 'c' testGame `shouldBe` (Game [('a', False)] [Player 0 "" Player.maxFaliures False, p2] True p2 "bc" ,True)
